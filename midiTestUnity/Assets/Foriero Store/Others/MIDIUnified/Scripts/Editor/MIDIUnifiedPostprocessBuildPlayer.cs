...
public class MIDIUnifiedPostprocessBuildPlayer : ScriptableObject
{
	...
	static void OnPostprocessBuildPlayer (BuildTarget target, string buildPath)
	{
		...
		if (target == BuildTarget.iOS) {
			...
			if (EditorUtility.DisplayDialogComplex ("Copy SF2?", @"Copy Plugins/MIDIUnified/iOS/softsynth.sf2 into build?", "Yes", "No", "") == 0) {
				sf2PathSource = Path.Combine (pluginsFolder, "iOS/softsynth.sf2");
				string fileName = Path.GetFileName(sf2PathSource);
		...
		if (target == BuildTarget.StandaloneLinux || target == BuildTarget.StandaloneLinux64 || target == BuildTarget.StandaloneLinuxUniversal) {
			...
			sf2PathSource = Path.Combine (pluginsFolder, "softsynth.sf2");
			...
			if (EditorUtility.DisplayDialogComplex ("Copy SF2?", @"Copy Plugins/MIDIUnified/Linux/softsynth.sf2 into build?", "Yes", "No", "") == 0) {

				sf2PathSource = Path.Combine (pluginsFolder, "Linux/softsynth.sf2");
				sf2PathDestination = Path.GetDirectoryName (buildPath) + "/softsynth.sf2";
				if (File.Exists (sf2PathSource)) {
					File.Copy (sf2PathSource, sf2PathDestination, true);
				} else {
		...
		if (target == BuildTarget.StandaloneOSXIntel || target == BuildTarget.StandaloneOSXIntel64 || target == BuildTarget.StandaloneOSXUniversal) {
			sf2PathSource = Path.Combine (pluginsFolder, "OSX/softsynth.sf2");
			sf2PathDestination = buildPath + "/Contents/Plugins/softsynth.bundle/Contents/Resources/softsynth.sf2";

			if (EditorUtility.DisplayDialogComplex ("Copy SF2?", @sf2PathSource + System.Environment.NewLine + @sf2PathDestination, "Yes", "No", "") == 0) {
		...
		if (target == BuildTarget.StandaloneWindows || target == BuildTarget.StandaloneWindows64) {
			...
			if (EditorUtility.DisplayDialog ("Copy SF2?", @"Copy MIDIUnified/Plugins/WIN/softsynth.sf2 into build?", "Yes", "No")) {

				sf2PathSource = Path.Combine (pluginsFolder, "WIN/softsynth.sf2");
				if (File.Exists (sf2PathSource))
					File.Copy (sf2PathSource, sf2PathDestination, true);
